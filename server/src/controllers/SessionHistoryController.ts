...
export class SessionHistoryController {
  ...
  async addFile(req: Request, res: Response): Promise<void> {
    try {
      ...
      const file = await this.service.addFileToHistory(
        sessionId,
        shortUrl,
        fileName,
        fileSize,
   ...
   */
  async addText(req: Request, res: Response): Promise<void> {
    try {
      const sessionId = req.cookies?.session_id;
      ...
      const text = await this.service.addTextToHistory(
        sessionId,
        shortUrl,
        title,
        expiresInDays
   ...
   */
  async removeText(req: Request, res: Response): Promise<void> {
    try {
      const sessionId = req.cookies?.session_id;
