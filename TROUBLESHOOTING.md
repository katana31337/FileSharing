...
### 0. Определите вашу среду

**Production установка** (сервер, `/opt/fileshare`):
```bash
cd /opt/fileshare
sudo bash full-diagnose.sh
```

**Разработка** (локальный компьютер, git pull):
```bash
cd /path/to/FileSharing
bash full-diagnose.sh
```

> 💡 **Примечание:** Все скрипты диагностики работают из любой директории проекта. Они автоматически определяют, где находится `docker-compose.yml`.
